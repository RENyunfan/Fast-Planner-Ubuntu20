...
void SDFMap::initMap(ros::NodeHandle &nh) {
    ...
    pc_sub_.reset(new message_filters::Subscriber<sensor_msgs::PointCloud2>(node_, "/sdf_map/cloud", 50));
...
void SDFMap::insertPointCloud() {
    md_.proj_points_.resize(md_.pc_.size());
    md_.proj_points_cnt = 0;
...
void SDFMap::pcOdomCallback(const sensor_msgs::PointCloud2ConstPtr &img, const nav_msgs::OdometryConstPtr &odom) {
    /* get pose */
    md_.camera_pos_(0) = odom->pose.pose.position.x;
    md_.camera_pos_(1) = odom->pose.pose.position.y;
    md_.camera_pos_(2) = odom->pose.pose.position.z;
